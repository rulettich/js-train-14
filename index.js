 ...
 * }
 */
function nextNewYear(currentDate) {
...
console.log(nextNewYear(currentDate));
//Виведе
...
function addDays(date, days) {
  ...
  const resultDateString = resultDate.toISOString();

  return {
    inputDate,
    addedDays: days,
    resultDate: resultDateString,
  };
}
...
function getDaysDifference(startDate, endDate) {
  ...
  const millisecondsDiff = endDateTime - startDateTime;

  const starDateString = startDate.toISOString();
  const endDateString = endDate.toISOString();

  const daysDifference = millisecondsDiff / (1000 * 60 * 60 * 24);

  return {
    starDate: starDateString,
